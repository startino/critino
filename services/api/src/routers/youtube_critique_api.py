...
def generate_critiques(chunks: List[str], context: str, query: str, optimal_response: str) -> List[str]:
...
@app.post("/generate-critique", response_model=CritiqueResponse)
def generate_critique(request: CritiqueRequest):
    ...
    print(chunks)
    critiques = generate_critiques(chunks, request.context, request.query, request.optimal_response)
